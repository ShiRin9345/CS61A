...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    typed_words_num, source_words_num, correct_words_num = len(typed_words), len(source_words), 0
    if typed_words_num == 0 or source_words_num == 0:
        return 100.0 if typed_words_num == 0 and source_words_num == 0 else 0.0

    for i in range(min(source_words_num, typed_words_num)):
        if typed_words[i] == source_words[i]:
            correct_words_num += 1
    return correct_words_num / typed_words_num * 100
    # END PROBLEM 3
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        # BEGIN PROBLEM EC
        args = (typed, source, limit)
        if args[:2] not in cache or cache[args[:2]][1] < limit:
            result = diff_function(*args)
            cache[args[:2]] = (result, limit)
            return result
        return cache[args[:2]][0]
...
def time_per_word(words, timestamps_per_player):
    ...
    for player in range(len(tpp)):
        ...
        for i in range(len(tpp[player]) - 1):
            time.append(tpp[player][i + 1] - tpp[player][i])
        times.append(time)
    # END PROBLEM 9
    ...
    for player in range(len(times)):
        ...
        for i in range(len(words)):
            ...
            for other in range(len(times)):
                if other == player:
                    continue
                if times[other][i] < times[player][i] or (times[other][i] == times[player][i] and other < player):
                    fastest = False
                    break
            if fastest:
                fastest_word.append(words[i])
        fastest_words_list.append(fastest_word)
