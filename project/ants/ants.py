...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        count = 0
        pos = self.place
        while not pos.is_hive:
            if pos.bees and  self.lower_bound <= count <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        self.reset_bees(amount)
        if self.health - amount <= 0:
            self.reset_bees(self.damage)
        super().reduce_health(amount) # 不需要传入self

    def reset_bees(self, damage):
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if ant:
            ant.action(gamestate) # 不需要self参数
        # END Problem 8a
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if not hasattr(target, 'slowed_time'):
            target.slowed_time = 0
        target.slowed_time += slow_duration

        def slowed_action(gamestate):
            if gamestate.time % 2 == 0 and target.slowed_time > 0:
                original_action(gamestate)
                target.slowed_time -= 1
            elif gamestate.time % 2 == 1 and target.slowed_time > 0:
                target.slowed_time -= 1
                return 
            else:
                original_action(gamestate)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while place.is_hive == False:
            if place.ant is not None and not isinstance(place.ant, LaserAnt):
                if place.ant is ContainerAnt and isinstance(place.ant.ant_contained, LaserAnt):
                    ans[place.ant.ant_contained] = distance
                ans[place.ant] = distance
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        ans_damage = self.damage - distance * 0.25 - self.insects_shot * 0.0625 
        print(f'DEBUG: insects_shot is: {self.insects_shot}')
        return ans_damage if ans_damage > 0 else 0
        # END Problem EC 4
